  ...
  table.config = function (key, value) {
	...
	table.render = function() {
		...
		for (var rowNum = 0, row; row = rows[rowNum]; rowNum++) {
			...
			var renderedRow = row.render(columnWidths, padding);
			...
			if (rowNum === 0) {
				...
				if (drawSeparator) {
					...
					for (var columnNum = 0, len = columnWidths.length; columnNum < len; columnNum++) {
						if (!isNaN(columnWidths[columnNum])) {
							var columnWidth = columnWidths[columnNum];
							result += horizontal.repeat(columnWidth);
							if (rowNum + 1 === columnWidths.length) {
								break;
							}
