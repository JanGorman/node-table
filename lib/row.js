module.exports = function() {
	...
	row.createColumn = function(content, options) {
		...
		if (options !== null) {
			['align', 'colspan'].forEach(function(conf) {
				switch (conf) {
					case 'align':
						align = options[conf];
						break;
					case 'colspan':
						colspan = options[conf];
						break;
				}
			});
		};
	...
	row.render = function(columnWidths, padding) {
		...
		for (var i = 0, column; column = columns[i]; i++) {
			...
			var slice = colNum + 1 === columnWidths.length ? columnWidths.slice(colNum) : columnWidths.slice(colNum, colspan);
			var columnWidth = (colspan - 1) + sum(slice);
		...
		if (colNum < columnWidths.length) {
			var remainingWidth = (columnWidths.length - colNum - 1) + sum(columnWidths.slice(colNum));
